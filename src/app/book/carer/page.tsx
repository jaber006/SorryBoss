...
export default function CarerBookingPage() {
  const router = useRouter();
  const [step, setStep] = useState(1); // 1: Your Details, 2: Care Recipient, 3: Time
  ...
  const validateStep2 = () => {
    if (!form.careRecipientName || !form.careRelationship) {
      setError("Please fill in the care recipient details");
      return false;
    }
    ...
    if (!form.symptomStartDate) {
      setError("Please tell us when symptoms started");
      return false;
    }
    return true;
  };
  ...
  const isTimeSlotPast = (slot: string) => {
    ...
    // For today, check if time has passed (with 30 min buffer)
    const now = new Date();
    ...
    <main className="min-h-screen bg-[#FDF8EE]">
      ...
      <div className="max-w-2xl mx-auto px-6 py-16">
        ...
        <div className="flex items-center gap-3 mb-12">
          {["Type", "Your Details", "Care Recipient", "Time"].map((stepName, i) => (
        ...
        {step === 1 && (
          <div>
            ...
            <p className="text-[#6B6560] mb-10">
              We need your information as the person taking carer&apos;s leave.
            </p>
        ...
        {step === 2 && (
          <div>
            ...
            <p className="text-[#6B6560] mb-10">
              Tell us about the person who is unwell.
            </p>
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateForm("careRecipientName", e.target.value)}
                  className="w-full px-4 py-3 rounded-xl border border-black/10 focus:border-[#1A1A1A] focus:ring-2 focus:ring-[#1A1A1A]/10 outline-none transition-all"
              ...
              <div>
                <label className="block text-sm font-medium mb-2">Their relationship to you *</label>
                <div className="grid grid-cols-2 gap-2">
                  {RELATIONSHIPS.map((rel) => (
                    <button
                      key={rel}
                      onClick={() => updateForm("careRelationship", rel)}
                      className={`text-left px-4 py-3 rounded-xl border transition-all ${
                        form.careRelationship === rel
              ...
              <div>
                <label className="block text-sm font-medium mb-3">What&apos;s wrong with them? *</label>
                <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Any other details about their condition..."
                />
              </div>

              <div>
                <label className="block text-sm font-medium mb-2">When did their symptoms start? *</label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-sm font-medium mb-2">Days of care needed</label>
              ...
              <div className="p-4 rounded-xl bg-[#E8B931]/10 border border-[#E8B931]/20">
                <p className="text-sm text-[#6B6560]">
                  <strong>Note:</strong> During the phone call, the pharmacist may ask to briefly speak with the unwell person if they are able to. This helps with the assessment.
                </p>
              </div>
            </div>
          </div>
                ...
                <div className="grid grid-cols-4 sm:grid-cols-6 gap-2 max-h-80 overflow-y-auto p-1">
                  {TIME_SLOTS.map((time) => {
                    ...
                    return (
                      <button
                        key={time}
                        onClick={() => !isPast && updateForm("preferredTime", time)}
                        disabled={isPast}
